...
export async function getSignalHistory({ caseId, period = '7d' }, sql = getSql()) {
  ...
  return { protocol: detail.protocol, series: { key, signalFamily: family, primarySignalType: detail.case.primarySignal?.type || null }, period, anchorDate, evaluationMode:'RETROSPECTIVE', engineVersion:SIGNAL_ENGINE_VERSION, availability:{ requestedCalendarDays:days, canonicalSnapshots:dates.length, evaluableSnapshots:summary.evaluableObservations, notEvaluableSnapshots:summary.notEvaluableObservations, missingCalendarDays:Math.max(days-dates.length,0) }, observations, summary, cache:{ existingObservations:cached.length, newlyEvaluated:evaluated.length, persisted:evaluated.length, durationMs:Date.now()-started, dbQueries:3 } };
}
