...
function ResearchCard({ item, rank, onUpdate, updating, onOpenCase }) {
  ...
  return <article className={`research-case ${expanded ? 'is-expanded' : ''}`}>
    ...
    <div className="research-case-actions"><button type="button" className="research-open" onClick={() => onOpenCase?.(item.id)}>{'Open research →'}</button><button type="button" className="research-expand" onClick={() => setExpanded(!expanded)}>{expanded ? 'Quick view ↑' : 'Quick view ↓'}</button><StatusActions item={item} onUpdate={onUpdate} updating={updating} /></div>
...
export default function DailyResearchPage({ onOpenCase }) {
  ...
  return <section className="daily-research-page" aria-label="Daily Research Feed">
    ...
    {!loading && !error && cases.length > 0 && <div className="research-case-list">{cases.map((item, index) => <ResearchCard key={item.id} item={item} rank={index + 1} onUpdate={update} updating={saving === item.id} onOpenCase={onOpenCase} />)}</div>}
